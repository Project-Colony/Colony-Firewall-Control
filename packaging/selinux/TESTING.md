...
| nft, the fast-allow set | needs ring 0 up (the object installed as in the bpf/perf row) and `[ebpf] fast_allow = true`, then both units running. Fast-allow armed: `cfc status` shows fast_allow live, `sudo nft list set inet colony_firewall fast_allow` shows one element. Stop the daemon: the set is empty again | `cfc status` shows fast_allow off with an nft error as the reason, and the set stays empty; AVC on `iptables_exec_t` (execute) - the `netlink_netfilter_socket` nft needs is the filtering group's, already exercised by the first row |
