...
`.SRCINFO` to the draft release — so the artifacts below can simply be
downloaded from the release instead of regenerated by hand.
...
contents. The layout is **flat**: `postInstall` runs with the extracted
directory as its working directory, so every file it names must be
present under exactly that basename.
...
To reproduce the tarball locally:
...
tar --zstd -cf "colony-firewall-control-${V}-linux-x86_64.tar.zst" \
    ...
    -C docs ARCHITECTURE.md HARDENING.md TROUBLESHOOTING.md
```
