...
downloaded from the release instead of regenerated by hand. One trap:
GitHub renames dot-leading asset filenames, so the published asset is
`default.SRCINFO` (`gh release download -p '.SRCINFO'` matches
nothing). Download `default.SRCINFO` and rename it to `.SRCINFO`
before copying it into the AUR checkout — the AUR rejects a push
without a file of exactly that name.
